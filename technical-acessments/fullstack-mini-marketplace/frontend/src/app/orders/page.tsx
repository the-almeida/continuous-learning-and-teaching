  ...
  return (
    <div>
      ...
      <ul className="space-y-6" aria-label="Order list">
        {orders.map((order) => (
          <li key={order.id} className="bg-white rounded-lg border border-gray-200 overflow-hidden">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-4 bg-gray-50 border-b border-gray-200">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="font-bold text-indigo-600">${Number(order.totalAmount).toFixed(2)}</span>
              </div>
            </div>

            <table className="w-full text-sm" aria-label={`Items in order ${order.id.slice(0, 8)}`}>
              <thead>
